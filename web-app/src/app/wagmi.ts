...
const HOODI_INFURA_RPC_ENDPOINT = `https://hoodi.infura.io/v3/${INFURA_API_KEY}`;
const HARDHAT_LOCALHOST_RPC_ENDPOINT = "http://127.0.0.1:8545";

const { mainnet, sepolia, hoodi, hardhat_localhost } = ETH_CHAINS;
const chains = [mainnet, hoodi, sepolia, hardhat_localhost] as const;
const connectors = [injected()];
const storage = createStorage({ storage: cookieStorage || localStorage });
const transports = {
  [mainnet.id]: http(),
  [sepolia.id]: http(SEPOLIA_INFURA_RPC_ENDPOINT),
  [hoodi.id]: http(HOODI_INFURA_RPC_ENDPOINT),
